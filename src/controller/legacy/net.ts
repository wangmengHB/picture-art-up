...
export class TransformNet implements Model {
  ...
  async load(): Promise<void> {
    ...
    if (!DEFAULT_STYLES[this.style] || !DEFAULT_STYLES[this.style].ckptsUrl) {
      throw new Error('has no such style!');
    }
    if (this.variableDictionary[this.style] == null) {
      const checkpointLoader =
          new CheckpointLoader(DEFAULT_STYLES[this.style].ckptsUrl);
